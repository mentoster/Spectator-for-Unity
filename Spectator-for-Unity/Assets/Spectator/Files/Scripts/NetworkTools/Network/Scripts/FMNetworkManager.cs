...
public class FMNetworkManager : MonoBehaviour
{
    ...
    public class FMClientSettings
    {
        ...
        [Tooltip("(( true by default ))")] public bool AutoNetworkDiscovery = true;
    ...
    [TextArea(1, 10)] public string Status;
    ...
    [Tooltip(
        "Sync Transformation of Network Objects. # Both Server and Clients should have same number of NetworkObjects")]
    ...
    public void Action_SyncNetworkObjectTransform(byte[] _data)
    {
        ...
        if (Timestamp > LastReceivedTimestamp)
        {
            ...
            for (int i = 0; i < NetworkObjects.Length; i++)
            {
                ...
                NetworkTransform[i].rotation =
                    new Quaternion(_transform[3], _transform[4], _transform[5], _transform[6]);
            ...
            if (Client.IsConnected)
            {
                if (NetworkObjects.Length > 0)
                {
                    for (int i = 0; i < NetworkObjects.Length; i++)
                    {
                        ...
                        float step = (CurrentTimestamp - LastReceivedTimestamp) /
                                     (TargetTimestamp - LastReceivedTimestamp);
                        step = Mathf.Clamp(step, 0f, 1f);
                        NetworkObjects[i].transform.position = Vector3.Slerp(NetworkObjects[i].transform.position,
                            NetworkTransform[i].position, step);
                        NetworkObjects[i].transform.rotation = Quaternion.Slerp(NetworkObjects[i].transform.rotation,
                            NetworkTransform[i].rotation, step);
                        NetworkObjects[i].transform.localScale = Vector3.Slerp(NetworkObjects[i].transform.localScale,
                            NetworkTransform[i].localScale, step);
                    }
                }
            }
